...
export default function BoardScreen({ navigation }) {
  ...
  const loadColumns = async () => {
    ...
    setError('');
    try {
      const base = { page: 1, limit: 100, projectId: projectId || undefined, sprintId: sprintId || undefined };
      ...
      const T = normalizeResponse(t).items;
      const P = normalizeResponse(p).items;
      const R = normalizeResponse(r).items;
      const D = normalizeResponse(d).items;
      setColumns({ todo: T, in_progress: P, review: R, done: D });
    } catch (e) {
      setError(e?.response?.data?.message || 'Erreur lors du chargement du board');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
        <ActivityIndicator />
        <Text style={{ marginTop: 8 }}>Chargement du board…</Text>
      </View>
    );
  }
